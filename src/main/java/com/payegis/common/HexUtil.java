...
public class HexUtil {
	...
	public static String bytesToHex(byte[] bs, int off, int length) {
		if (bs.length <= off || bs.length < off + length) {
			System.out.println("bs.length :=" + bs.length + " / off+length := " + off + length);
			throw new IllegalArgumentException("bs.length :=" + bs.length + " / off+length := " + off + length);
		}
	...
	public static byte[] bitsToBytes(BitSet ba, int size) {
		...
		sb = new StringBuilder(8 * bytesAlloc); // TODO: Should it be
		// 2*8*bytesAlloc
		// here?
